...
int main()
{
    std::vector<std::vector<float>> train_X(60000, std::vector<float>(785, 0.f));
    load_data(60000, 785, train_X, "data/train.bin");

    std::vector<std::vector<float>> train_y = extract_label(train_X);

    std::vector<std::vector<float>> test_X(10000, std::vector<float>(785, 0.f));
    load_data(10000, 785, test_X, "data/test.bin");

    std::vector<std::vector<float>> test_y = extract_label(test_X);
    ...
    int pred, trg;
    float mean_loss, mean_acc, test_acc;
    ...
    for (int epoch = 0; epoch < 10; epoch++)
    {
        ...
        std::cout << "Epoch: " << epoch + 1 << "/10 "
                  << "Train/loss: " << mean_loss / (NUM_BATCH) << " "
                  << "Train/accuracy: " << mean_acc / (NUM_BATCH * BATCH_SIZE) << " ";
        ...
        for (int i = 0; i < 10000; i++)
        {
            ...
            pred = argmax(a);
            trg = argmax(y);
            test_acc += int(pred == trg);
        }
        std::cout << "Test/accuracy: " << test_acc / 10000 << "\n";
    }
}